...
class CMT5Request
{
    ...
    public function Init($server)
    {
        ...
        //curl_setopt($this->m_curl,CURLOPT_FORBID_REUSE,1); // force the connection to explicitly close
    ...
    public function Get($path)
    {
        ...
        if($result==false)
        {
            $this->E[] = 'Curl GET error: '.curl_error($this->m_curl);
            return(false);
        }
        $code=curl_getinfo($this->m_curl,CURLINFO_HTTP_CODE);
        if($code!=200)
        {
            $this->E[] = 'Curl GET code: '.$code;
            return(false);
        }
        return($result);
    }
    ...
    public function Post($path, $body)
    {
        ...
        if($result==false)
        {
            $this->E[] ='Curl POST error: '.curl_error($this->m_curl);
            return(false);
        }
        $code=curl_getinfo($this->m_curl,CURLINFO_HTTP_CODE);
        if($code!=200)
        {
            $this->E[] ='Curl POST code: '.$code;
            return(false);
        }
        return($result);
    }
    ...
    public function Auth($login, $password, $build, $agent)
    {
        ...
        {
            $this->E[] ='Auth start error : '.$auth_start_answer->retcode;
            return(false);
        }
        ...
        {
            $this->E[] ='Auth answer error : '.$auth_answer_answer->retcode;
            return(false);
        }
        ...
        {
            $this->E[] ='Auth answer error : invalid client answer';
            return(false);
        }
        //--- Everything is done
        return(true);
