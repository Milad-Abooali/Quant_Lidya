...
if ($request->Init('mt5.fx-technology.com:443') && $request->Auth(1000, "@Sra7689227", 3500, "WebManager"))
{
	...
	//$result=$request->Get('/trade_balance?login=100395&type=6&balance=-'.$json->answer->Credit.'&comment=test');
	if($result!=false)
	{	
		//echo $result;
		//echo "</br></br>";
		...
		//echo "</br></br>";
		for ($i = 0; $i < $total; $i++){
    		//echo "Login: ".$json->answer[$i]->Login;
    		//echo "</br>";
    		//echo "Balance: $".$json->answer[$i]->Balance;
    		//echo "</br>";
    		//echo "Equity: $".$json->answer[$i]->Equity;
    		//echo "</br>";
    		//echo "Profit: $".$json->answer[$i]->Profit;
    		//echo "</br>";
    		//echo "Margin Level: %".$json->answer[$i]->MarginLevel;
    		//echo "</br>";
    		//echo "Credit: $".$json->answer[$i]->Credit;
    		//echo "<br></br>";
    		if($json->answer[$i]->Balance < 0 ){
    		    if($json->answer[$i]->Equity > 0){
            		if($json->answer[$i]->Credit > 0){
            		    $result=$request->Get('/trade_balance?login='.$json->answer[$i]->Login.'&type=6&balance=-'.$json->answer[$i]->Credit.'&comment=Credit%20Out&check_margin=0');
            		}
    		    } else {
    		        if($json->answer[$i]->Credit > 0){
            			$result=$request->Get('/trade_balance?login='.$json->answer[$i]->Login.'&type=6&balance=-'.$json->answer[$i]->Credit.'&comment=Credit%20Out&check_margin=0');
            			if($result!=false)
            			{	
            				$result=$request->Get('/trade_balance?login='.$json->answer[$i]->Login.'&type=2&balance='.abs($json->answer[$i]->Balance).'&comment=Zeroing');
            			}
            		} else {
            		    $result=$request->Get('/trade_balance?login='.$json->answer[$i]->Login.'&type=2&balance='.abs($json->answer[$i]->Balance).'&comment=Zeroing');
            		}
