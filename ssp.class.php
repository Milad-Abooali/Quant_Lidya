...
class SSP {
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '')
    {
        ...
        $resTotalLOT = SSP::sql_exec( $db,
            "SELECT SUM(VOLUME)
			FROM MT4_TRADES AS MT4_TRADES LEFT JOIN MT4_USERS AS MT4_USERS ON (MT4_TRADES.LOGIN = MT4_USERS.LOGIN) $extraWhere"
        );
        
        $resTotalPROFIT = SSP::sql_exec( $db,
            "SELECT SUM(PROFIT)
			FROM MT4_TRADES AS MT4_TRADES LEFT JOIN MT4_USERS AS MT4_USERS ON (MT4_TRADES.LOGIN = MT4_USERS.LOGIN) $extraWhere"
        );
        
        $resTotalSWAP = SSP::sql_exec( $db,
            "SELECT SUM(SWAPS)
			FROM MT4_TRADES AS MT4_TRADES LEFT JOIN MT4_USERS AS MT4_USERS ON (MT4_TRADES.LOGIN = MT4_USERS.LOGIN) $extraWhere"
        );
        
        $resLOT = $resTotalLOT[0][0];
        $resPROFIT = $resTotalPROFIT[0][0];
        $resSWAP = $resTotalSWAP[0][0];
        ...
        return array(
            ...
            "recordsTotalLOT" => floatval($resLOT),
            "recordsTotalPROFIT" => floatval($resPROFIT),
            "recordsTotalSWAP" => floatval($resSWAP),
